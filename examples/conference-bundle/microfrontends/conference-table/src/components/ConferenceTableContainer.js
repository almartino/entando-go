...
class ConferenceTableContainer extends Component {
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { conferences, headers } = await apiConferencesGet(serviceUrl, requestParameters);
        const conferenceCount = (headers && headers['X-Total-Count']) || 0;
        console.log(`conferences`, conferences, conferenceCount)
        this.dispatch({
          type: READ_ALL,
          payload: {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiConferencesDelete(serviceUrl, item.id);
        onDelete(item);
